import { Box, Button, Dialog, DialogActions, DialogContent, DialogContentText, InputLabel, TextField, TextareaAutosize, Typography, DialogTitle, Backdrop, Snackbar } from "@mui/material"
  ...
  return (
    <>
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id="alert-dialog-description" fontWeight={"bold"}>
            Do you really want to transfer {toRupiah(Number(amount), { dot: ",", floatingPoint: 0 })} from {data?.data?.user} ??
          </DialogContentText>
        </DialogContent>
        <DialogActions>
      ...
      <Box width={"100%"} display={"flex"} justifyContent={"center"} alignItems={"center"} flexDirection={"column"}>
        <Box display={"flex"} gap={1} alignItems={"center"} onClick={() => navigate(-1)} width={"100%"} pt={2}>
        ...
        <Box width={"100%"} p={3} display={"flex"} justifyContent={"center"} alignItems={"flex-start"}>
          <Box width={"50%"} flexDirection={"column"} display={"flex"} justifyContent={"center"}>
            ...
            <Box display={"flex"} width={"100%"} flexDirection={"column"}>
              ...
              <Box>
                {err?.data?.message !== "" && <Typography color={"red"}>{err?.data?.message}</Typography>}
              </Box>
            </Box>
            {err?.data?.statusCode === 404 && (
              <Box width={"100%"} display={"flex"} justifyContent={"center"} alignItems={"center"} p={8}>
                <Typography color={"red"} fontWeight={"bold"} fontSize={"20px"}>{err.data.message}</Typography>
              </Box>
            )}
            {isSuccess && data?.data != null ? (
              <>
                <Box width={"100%"} p={2} height={"200px"} borderRadius={"20px"} border={"1px solid #333"} my={5} flexDirection={"column"} display={"flex"}>
                  <img src={data.data.photo_profile} style={{ borderRadius: "100%" }} width={"40px"} height={"40px"} />
                  <Typography mt={1} fontWeight={"bold"}>User : <span style={{ fontWeight: "" }}>{data.data.user}</span></Typography>
                  <Typography>Name : {data.data.name}</Typography>
                  <Typography>Account number : {data.data.accountNumber}</Typography>
                  <Typography>Created at : {timeStampToLocaleString(data.data.created_at)}</Typography>
                </Box>
              </>
            ) : null}
          </Box>
          <Box width={"50%"} flexDirection={"column"} display={"flex"} alignItems={"center"} justifyContent={"center"}>
            <Box width={"70%"} display={"flex"} flexDirection={"column"} alignItems={"flex-start"} justifyContent={"center"}>
              <Box width={"100%"}>
                <InputLabel>Amount : </InputLabel>
                <TextField placeholder="Rp. 0" value={amount} fullWidth onChange={(e: any) => {
                  setAmount(e.target.value)
                }} />
              </Box>
              {errAmount?.data?.message !== "" && <Typography color={"red"}>{errAmount?.data?.message}</Typography>}
              <Box width={"100%"} pt={3}>
                <InputLabel>Notes : </InputLabel>
                <TextareaAutosize placeholder={`example: Hello ${data?.data?.user == null ? "" : data.data.user}`} style={{ width: "100%" }} value={notes} minRows={7} onChange={(e: any) => {
                  setNotes(e.target.value)
                }} />
              </Box>
            </Box>
            <Box width={"70%"} mt={2} p={2} display={"flex"} flexDirection={"column"} border={"1px solid black"} borderRadius={"10px"}>
              <Typography fontWeight={"bold"} color={"green"}>Information :</Typography>
              ...
              <Typography>Current balance : {toRupiah(user.balance, { dot: ",", floatingPoint: 0 })}</Typography>
              <Typography>Amount : {toRupiah(amount == "" ? 0 : amount, { dot: ",", floatingPoint: 0 })}</Typography>
              <Typography>Notes : {notes}</Typography>
            </Box>
            <Box width="70%" display={"flex"} py={2} justifyContent={"flex-end"}>
              <Button variant="contained" disabled={isSuccess ? false : true} color="success" onClick={() => {
