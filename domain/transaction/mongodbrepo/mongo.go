...
type TransferInfo struct {
	...
	Dto         dto.TransferRequest
	Money       *entity.MoneyAfter
}

func (t *MongoTransferStore) InsertTransferHistory(payload *TransferInfo) error {
...
func (m *MongoTopUpStore) InsertTopUpHistory(payload *TopUpInfo) error {
	ctx, cancel := context.WithTimeout(context.Background(), 4*time.Second)
	defer cancel()
	fmt.Println(payload)
