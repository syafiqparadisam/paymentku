...
func (s *MySqlStore) IncreaseBalanceByNameAndEmail(amount uint32, user, email string) error {
...
func (s *MySqlStore) FindBalanceByName(user string) (*UserIdAndBalance, error) {
	// query to find user
...
func (s *MySqlStore) DecreaseBalanceByUser(amount uint32, user string) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
...
func (s *MySqlStore) IncreaseBalanceByAccNumber(amount uint32, accNumber int64) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
...
func (s *MySqlStore) FindNameAccNumAndEmailByAccNum(accNumber int64) (*UserAccNumAndEmail, error) {
...
func (s *MySqlStore) FindUserAndEmailByAccNumber(accNumber int64) (*UserAndEmail, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
...
func (s *MySqlStore) FindAccNumByName(name string) (*AccountNumber, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
