...
const HistoryTopupById = () => {
  ...
  const { data: data, isSuccess } = useGetHistoryTopUpByIdQuery(
    parseInt(id ? id : "0")
  );
        ...
        <Box m={3} width={"100%"}>
          ...
          {isSuccess ? (
            <Box
              ...
              display={"flex"}
              bgcolor={
                data?.data?.status === "SUCCESS" && data.data.status != null
                  ? "lightgreen"
                  : "red"
              }
            ...
            >
              ...
              >
                <Delete
                  fontSize="medium"
                  sx={{
                    color: data?.data?.status == "SUCCESS" ? "black" : "white",
                  }}
                />
              </Box>
              <Box display={"flex"}>
              ...
              >
                <Typography fontSize={"15x"} fontWeight={"bold"}>
                  {"Amount : " +
                    toRupiah(
                      data?.data?.amount == null ? 0 : data.data.amount,
                      { dot: ",", floatingPoint: 0 }
                    )}
                </Typography>
                <Typography fontSize={"15x"} fontWeight={"bold"}>
                  {"Current Balance : " +
                    toRupiah(
                      data?.data?.balance == null ? 0 : data.data.balance,
                      { dot: ",", floatingPoint: 0 }
                    )}
                </Typography>
                <Typography fontSize={"15x"} fontWeight={"bold"}>
                  {"Previous Balance : " +
                    toRupiah(
                      data?.data?.previousBalance == null
                        ? 0
                ...
                <TimeAgoComponent
                  timestamp={data?.data?.createdAt ? data?.data?.createdAt : ""}
                  color={
                    data?.data?.status === "SUCCESS" && data.data.status != null
                      ? "black"
                      : "white"
